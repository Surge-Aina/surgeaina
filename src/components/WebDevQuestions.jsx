...
export default function WebDevQuestions({ onAnswersChange, onContinue }) {
  ...
  const canProceedFromStep2 = () => {
    const hasDesignAnswer = answers.designs && answers.designs.length > 0;
    const hasPlatformAnswer = answers.platform && answers.platform.length > 0;
    return hasDesignAnswer && hasPlatformAnswer;
  };

  const canProceedFromStep3 = () => {
    const hasAdditionalHelpAnswer = answers.additional_help && answers.additional_help.length > 0;
    const hasProjectScopeAnswer = answers.project_scope && answers.project_scope.length > 0;
    const hasTimelineAnswer = answers.timeline && answers.timeline.length > 0;
  ...
  if (!questions || !Array.isArray(questions)) {
    return (
      <div className="text-center py-8">
        <p className="text-red-600">Error loading questions for Web Development</p>
      </div>
    );
  }
  ...
  // Step 1: Show only first question
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {firstQuestion.options.map((option, index) => {
              ...
              const isSelected = (answers.type || []).includes(optionValue);
              ...
              return (
                <label key={index} className="cursor-pointer block">
                  <input
                    ...
                    checked={isSelected}
                    onChange={(e) => {
                      handleCheckboxChange('type', optionValue, e.target.checked);
                    }}
                    className="sr-only"
                  />
  ...
  // Step 2: Show questions 2 and 3 together
  ...
  // Step 3: Show remaining questions (4, 5, 6)
  if (currentQuestionStep === 3) {
    const questionsToShow = questions.slice(3); // Get questions 4, 5, 6
      ...
      <div className="space-y-8">
        ...
        <div className="border-t border-gray-200 pt-6">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 mt-2">
              {canProceedFromStep3() ? 'Ready to continue' : 'Please answer all questions above'}
            </p>
          </div>
        </div>
      </div>
